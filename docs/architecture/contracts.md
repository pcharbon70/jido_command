...
- `signal_bus.name` must be a non-empty string/atom when provided; binary values are normalized by trimming whitespace and an optional leading `:`, and must not normalize to empty
